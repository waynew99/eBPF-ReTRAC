...
echo "starting prob.sh"

# check if we have $1 and $2
if [ $# -ne 2 ]; then
    echo "Usage: ./prob.sh <batch_size> <num_freeports>"
    exit 1
fi
batch_size=$1
num_freeports=$2
...
for ((i=0; i<trials; i++)); do
    ...
    printf "%d,%d,%d,%.6f,%.6f\n" "$batch_size" "$num_freeports" "$success_code" "$time_total" "$time_connect" >> ../results/prob_results.csv
done
